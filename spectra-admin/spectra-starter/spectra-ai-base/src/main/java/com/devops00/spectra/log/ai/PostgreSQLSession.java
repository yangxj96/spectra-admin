//package com.devops00.spectra.log.ai;
//
...
// * <p>
// * This implementation stores session state in PostgreSQL database tables.
// * </p>
// * <p>
...
// *   PRIMARY KEY (session_id, state_key, item^_index)
// * );
// *
...
// * BEFORE UPDATE ON agentscope_sessions
// * FOR EACH ROW
...
//     */
//    public PostgreSQLSession(DataSource dataSource, boolean createIfNotExist) {
...
//        String createTableSql = "CREATE TABLE IF NOT EXISTS " + getFullTableName() + " ("
//                + "session_id VARCHAR(255) NOT NULL, "
//                + "state_key VARCHAR(255) NOT NULL, "
//                + "item_index INT NOT NULL DEFAULT 0, "
//                + "state_data TEXT NOT NULL, "
...
//            // 如果能获取连接，通常说明数据库存在
//        } catch (SQLException e) {
//            throw new IllegalStateException("Cannot connect to database: " + databaseName, e);
//        }
//    }
//
//    /**
...
//        String checkSql = "SELECT COUNT(*) FROM " + getFullTableName() + " LIMIT 1";
//        try (Connection conn = dataSource.getConnection();
//             PreparedStatement stmt = conn.prepareStatement(checkSql)) {
//            stmt.execute();
//        } catch (SQLException e) {
//            throw new IllegalStateException("Table does not exist: " + getFullTableName(), e);
//        }
//    }
//
//    /**
...
//        // 注意：在PostgreSQL中，通常使用Schema(如public)而不是Database名来拼接表名。
//        // 但为了保持原有逻辑兼容性，这里仍使用双引号包裹。
...
//     */
//    private void executeInWriteTransaction(Connection conn, SqlOperation operation) throws Exception {
...
//
//        // 修改点：PostgreSQL 的 Upsert 语法
//        // 注意：PostgreSQL 需要明确指定 ON CONFLICT 的索引目标
//        String upsertSql = "INSERT INTO " + getFullTableName() + " (session_id, state_key, item_index, state_data) "
//                + "VALUES (?, ?, ?, ?) "
//                + "ON CONFLICT (session_id, state_key, item_index) " // 指定主键冲突
//                + "DO UPDATE SET state_data = EXCLUDED.state_data"; // EXCLUDED 引用新值
//
//        try (Connection conn = dataSource.getConnection()) {
//            executeInWriteTransaction(conn, () -> {
