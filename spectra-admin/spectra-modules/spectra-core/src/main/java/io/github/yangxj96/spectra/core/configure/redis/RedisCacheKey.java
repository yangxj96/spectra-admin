...
public final class RedisCacheKey {
    ...
    public static final String AUTH_LOGIN_TYPE = "auth:login:user:active_token:%s:%s";

    /// token信息,存储token的具体信息(包含扩展信息)
    /// * 格式:authorization:{token}
    /// * 示例:authorization:token:02038A56-527D-498D-8E89-71CCCBAD6B36
    public static final String AUTH_TOKEN = "auth:token:detail:%s";
    ...
    public static final String AUTH_SECURITY = "auth:security:token:detail:%s";
    ...
    public static final String AUTH_TOKEN_USER = "auth:token:user:resolve:%s";

    /// 用户所有活跃登录方式集合（用于管理页面）,内容是set
    /// * 格式:authorization:user_login_types:{user_id}
    /// * 示例:authorization:user_login_types:1999056543696211969
    public static final String AUTH_USER_LOGIN_TYPES = "auth:user:login:active_types:%s";
}
