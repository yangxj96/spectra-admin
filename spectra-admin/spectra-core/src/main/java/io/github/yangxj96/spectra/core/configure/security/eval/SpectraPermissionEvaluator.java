...
public class SpectraPermissionEvaluator implements PermissionEvaluator {
    ...
    /**
     * 权限表达式缓存（线程安全 LRU） <br/>
     * key: 原始权限表达式，如 "order:*:read" <br/>
     * value: 预编译表示，例如分段结构或 regex Pattern <br/>
     */
    ...
    /**
     * 预编译后的权限表达式结构 <br/>
     * 若为 segmentBased=true，则使用分段匹配（最快） <br/>
     * 若为 segmentBased=false，则 fallback 使用 regex <br/>
     *
     * @param segmentBased
    ...
    /**
     * 判断用户是否拥有绝对权限: <br/>
     * - ADMINISTRATORS（即 ROLE_DEV_OPS） <br/>
     * - 或权限字符串为 "*"（全权限） <br/>
     */
    ...
    /**
     * 编译权限表达式。 <br/>
     * 如果表达式包含 ":" → 使用分段匹配（性能最佳） <br/>
     * 否则 → fallback 为 regex <br/>
     */
     ...
     * <p>
     * 示例：<br/>
     * pattern:  order:*:read<br/>
     * user:     order:12:read<br/>
     * <p>
     * pattern:  order:**            (多级通配)<br/>
     * user:     order:12:item:edit<br/>
     */
