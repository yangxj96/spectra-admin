...
@Slf4j
@Component
public class ServiceMonitorServiceSupport implements ServiceMonitorService {
    ...
    private final Supplier<Collection<Timer>> requestTimers;
    ...
    public ServiceMonitorServiceSupport(MeterRegistry meterRegistry, TimeMapper timeMapper, Environment environment,
        ...
        this.requestTimers = () -> meterRegistry.find("http.server.requests").timers();
    ...
    private void collectSnapshotInternal() {
        try {
            ...
            var evaluation = alertService.evaluateWithResult(persisted);
            if (evaluation.hasFailures()) {
                log.warn("服务监控告警评估包含规则级失败: failedRules={}", evaluation.failures());
            }
    ...
    private RequestMetrics collectRequestMetrics() {
        Collection<Timer> timers = requestTimers.get();
        if (timers.isEmpty()) {
