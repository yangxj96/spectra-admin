...
public class HmacAuthorizationChangeTokenService implements AuthorizationChangeTokenService {
    ...
    public String issue(AuthorizationChangeToken token) {
        ...
        String payload = String.join("|", "1", token.tokenId().toString(), token.operatorId().toString(),
                token.targetUserId().toString(), token.roleId().toString(), token.assignmentId().toString(),
    ...
    public AuthorizationChangeToken verify(String encodedToken) {
        ...
        try {
            ...
            return new AuthorizationChangeToken(UUID.fromString(values[1]), UUID.fromString(values[2]),
                    UUID.fromString(values[3]), UUID.fromString(values[4]), UUID.fromString(values[5]),
