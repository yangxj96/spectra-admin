...
public class DatabaseSecurityUserLoader implements SecurityUserLoader {
     ...
     * @param securityUserAssembler         将用户、认证身份和权限快照组装为安全主体
     */
    public DatabaseSecurityUserLoader(
                                      ...
                                      SecurityUserAssembler securityUserAssembler) {
    ...
    public @Nullable SecurityUser load(UUID userId) {
        ...
        try {
            return securityUserAssembler.toSecurityUser(LoginType.PASSWORD, identity, credential, user);
