...
class CacheManagementServiceTest {
    ...
    @Test
    void shouldRevokeOnlyTheSessionIdentifiedByItsOpaqueHandle() {
        var revocation = mock(SecuritySessionRevocationPort.class);
        var service = service(revocation, mock(SecurityReplayNonceAdminPort.class));

        service.revokeSingleSession(new SecuritySessionRevokeOneFrom("opaque-session-handle", "security review", true));

        verify(revocation).revokeSessionById("opaque-session-handle");
        verify(revocation, never()).revokeUserSessions(any());
    }

    @Test
    void shouldPreserveCurrentSessionWhenRevokingAllSessionsForCurrentUser() {
        var revocation = mock(SecuritySessionRevocationPort.class);
        var securityContext = mock(SecurityContextAccessor.class);
        UUID userId = UUID.randomUUID();
        String currentToken = "current-access-token";
        when(securityContext.currentUserId()).thenReturn(userId);
        when(securityContext.currentToken()).thenReturn(currentToken);
        var service = service(revocation, mock(SecurityReplayNonceAdminPort.class),
                mock(CacheInvalidationCoordinator.class), mock(SecurityVerificationCodeStore.class),
                mock(SecurityVerificationAttemptStore.class), securityContext);

        service.revokeAllSessions(new SecuritySessionRevokeAllFrom(userId, "security review", true));

        verify(revocation).revokeUserSessionsExceptToken(userId, currentToken);
        verify(revocation, never()).revokeUserSessions(userId);
    }

    @Test
    void shouldRevokeAllSessionsWhenTargetingAnotherUser() {
        var revocation = mock(SecuritySessionRevocationPort.class);
        var securityContext = mock(SecurityContextAccessor.class);
        UUID currentUserId = UUID.randomUUID();
        UUID targetUserId = UUID.randomUUID();
        when(securityContext.currentUserId()).thenReturn(currentUserId);
        var service = service(revocation, mock(SecurityReplayNonceAdminPort.class),
                mock(CacheInvalidationCoordinator.class), mock(SecurityVerificationCodeStore.class),
                mock(SecurityVerificationAttemptStore.class), securityContext);

        service.revokeAllSessions(new SecuritySessionRevokeAllFrom(targetUserId, "security review", true));

        verify(revocation).revokeUserSessions(targetUserId);
        verify(revocation, never()).revokeUserSessionsExceptToken(any(), any());
    }

    @Test
    void shouldRejectSelfRevokeAllWhenCurrentSessionCannotBeConfirmed() {
        var revocation = mock(SecuritySessionRevocationPort.class);
        var securityContext = mock(SecurityContextAccessor.class);
        UUID userId = UUID.randomUUID();
        when(securityContext.currentUserId()).thenReturn(userId);
        when(securityContext.currentToken()).thenReturn(null);
        var service = service(revocation, mock(SecurityReplayNonceAdminPort.class),
                mock(CacheInvalidationCoordinator.class), mock(SecurityVerificationCodeStore.class),
                mock(SecurityVerificationAttemptStore.class), securityContext);
    ...
    @Test
    void shouldRejectInvalidSingleSessionRevokeRequestsBeforeCallingSecurityRedis() {
        var revocation = mock(SecuritySessionRevocationPort.class);
        var service = service(revocation, mock(SecurityReplayNonceAdminPort.class));

        assertThrows(IllegalArgumentException.class,
                () -> service.revokeSingleSession(new SecuritySessionRevokeOneFrom(" ", "reason", true)));
        assertThrows(IllegalArgumentException.class,
                () -> service.revokeSingleSession(new SecuritySessionRevokeOneFrom("handle", " ", true)));
        assertThrows(IllegalArgumentException.class,
                () -> service.revokeSingleSession(new SecuritySessionRevokeOneFrom("handle", "x".repeat(201), true)));
        assertThrows(IllegalArgumentException.class,
                () -> service.revokeSingleSession(new SecuritySessionRevokeOneFrom("handle", "reason", false)));

        verify(revocation, never()).revokeSessionById(any());
    }
    ...
    private static CacheManagementServiceImpl service(SecuritySessionRevocationPort revocation,
        ...
        return service(revocation, nonce, coordinator, codes, attempts, mock(SecurityContextAccessor.class));
    }
    ...
    private static CacheManagementServiceImpl service(SecuritySessionRevocationPort revocation,
        ...
        return new CacheManagementServiceImpl(mock(CacheRegionRegistry.class), coordinator,
                mock(SecuritySessionQueryPort.class), revocation, securityContextAccessor, codes, attempts,
