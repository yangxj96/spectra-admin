...
/**
 * 邮件 Provider 的测试替身示例；只返回确定性的 Provider 结果，不访问网络且不记录地址或敏感内容。
 */
public final class MockEmailSender implements NotificationSender {
    ...
    public ChannelSendResult send(NotificationTaskEntity task) {
        return new ChannelSendResult("SENT", "MOCK_EMAIL", "mock-email-" + task.getId(), "Mock email accepted");
    }
}
