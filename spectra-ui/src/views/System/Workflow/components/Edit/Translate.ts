...
const translations = Object.assign(zh, {
    Process: "流程",
    Name: "名称",
    Value: "值",
    ID: "唯一标识(ID)",
...
const zhTranslate: TranslateFn = function (template, replacements) {
    replacements = replacements || {};
    let translated = translations[template] || template;
    return translated.replaceAll(/{([^}]+)}/g, function (_, key) {
        return replacements[key] === undefined ? "{" + key + "}" : replacements[key];
    });
};

export default {
    translate: ["value", zhTranslate] as const
};
