...
/// 1. 输入端：接收大模型生成的原始 JSON 字符串，利用项目统一的 Jackson 反序列化为强类型 DTO。
/// 2. 输出端：捕获业务执行结果，利用项目统一的 Jackson 序列化为标准的 JSON 字符串吐给大模型。
/// 3. 异常端：统一拦截业务及解析异常，转为大模型可读的 JSON 错误文本，防止 WebFlux 响应流中断。
///
/// @author Jack Young
/// @version 1.0
